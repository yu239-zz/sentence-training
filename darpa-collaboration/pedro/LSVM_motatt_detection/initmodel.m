...
if nargin < 5 | (nargin >=5 & ~numel(sbin))
  model.sbin = 8;
else
  model.sbin = sbin;
end
...
% size of root filter
if nargin < 6 | (nargin >=6 & ~numel(sz))
  sz = [round(h/model.sbin) round(w/model.sbin)];
end

% add root filter
[model, symbol, filter] = model_addfilter(model, zeros([sz dimz]), symmetry);
