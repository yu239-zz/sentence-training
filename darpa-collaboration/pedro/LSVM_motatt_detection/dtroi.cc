...
// dt of 1d array
void dt1d(double *src, double *dst, int *ptr, int step, int start, int n, 
	  double a, double b) {
  dt_helper(src, dst, ptr, step, start, n-1, start, n-1, a, b);
}

// matlab entry point
// [M, Ix, Iy] = dtroi(vals, ax, bx, ay, by, xvals, yvals)
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  double *xvals = (double *)mxGetPr(prhs[5]);  
  double *yvals = (double *)mxGetPr(prhs[6]);   
  const int *dimsroi = mxGetDimensions(prhs[5]);
 ...
 // fix this when time
  for (int i = 0; i < dimsroi[0]; i++) {
    int x = (int)xvals[i] - 1;
    dt1d(vals+x*dims[0], tmpM+x*dims[0], tmpIy+x*dims[0], 1, 0, dims[0], ay, by);
    }

  for (int y = 0; y < dims[0]; y++)
    dt1d(tmpM+y, M+y, tmpIx+y, dims[0], 0, dims[1], ax, bx);
