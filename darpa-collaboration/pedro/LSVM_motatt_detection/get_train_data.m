function [pos, neg] = get_train_data(cls, flippedpos, year)
...
try
  load(fullfile(cachedir, [cls '_train_' year '.mat']));
catch
  ...
  for i = 1:length(ids);
    ...
    % read in also the pose info
    clsinds = strmatch(cls, {rec.objects(:).class}, 'exact');
    % skip difficult examples
    diff = [rec.objects(clsinds).difficult];
    clsinds(diff) = [];
    for j = clsinds(:)'
      if isfield(rec.objects(j), 'pose') & numel(rec.objects(j).pose)
          numpos = numpos+1;
          ...
          pos(numpos).pose = rec.objects(j).pose;
          ...
          if flippedpos
                ...
                pos(numpos).y2 = bbox(4);
                pos(numpos).pose = rec.objects(j).pose;
                pos(numpos).pose(1) = 360 - pos(numpos).pose(1);
  ...
  save(fullfile(cachedir, [cls '_train_' year '.mat']), 'pos', 'neg');
end  
