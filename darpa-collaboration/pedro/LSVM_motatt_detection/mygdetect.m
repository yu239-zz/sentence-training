function [dets, boxes, info, pixelscoremaps] = mygdetect(pyra, model, thresh, bbox, overlap)
...
scoremaps = struct('score', {rules(:).score}, 'detwindow', {rules(:).detwindow}, 'partids', {rules(:).rhs},...
                   'anchor', {rules(:).anchor});
for i = 1 : length(scoremaps)
    scoremaps(i).score = {scoremaps(i).score{model.interval+1:length(pyra.scales)}};
end;
%pixelscoremaps = score2pixelmaps(pyra, scoremaps, model);
pixelscoremaps = [];
...
n = ceil(length(filters) / max_filters);
...
for level = levels
  ...
  r = cell(length(filters), 1);
  pntr = 1;
  pdimy = size(pyra.feat{level}, 1);
  pdimx = size(pyra.feat{level}, 2);
  for j = 1 : n
     pntr_end = min(length(filters), pntr + max_filters - 1);
     r_temp = fconv(pyra.feat{level}, {filters{pntr:pntr_end}}, 1, pntr_end - pntr + 1);
     for k = 1 : pntr_end - pntr + 1
        rk = r_temp{k};
        rk = padarray(rk, [pdimy - size(rk, 1)+1, pdimx - size(rk, 2)+8], sum(sum(filters{pntr+k-1}(:,:,32))), 'post');
        %r{pntr+k-1} = r_temp{k};
        r{pntr+k-1} = rk;
     end;
     pntr = pntr_end + 1;
