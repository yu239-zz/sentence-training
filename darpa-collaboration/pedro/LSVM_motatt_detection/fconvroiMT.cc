...
void *process(void *thread_arg) {
  ...
  double *B = args->B;
  double *C = args->C;
  double *xvals = args->xvals;
  double *yvals = args->yvals;
...
for (int f = 0; f < num_features; f++) {
    ...
    for (int i = 0; i < dims[0]; i++) {
      int x = (int)xvals[i] - 1;  // subtract 1 for matlab
      int y = (int)yvals[i] - 1;
      if (x < C_dims[1] && y < C_dims[0]) {
	double val = 0;
...
// matlab entry point
// C = fconvroi(A, cell of B, start, end, xvals, yvals);
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  double *xvals = (double *)mxGetPr(prhs[4]);  
  double *yvals = (double *)mxGetPr(prhs[5]);   
  const int *dimsroi = mxGetDimensions(prhs[4]);
