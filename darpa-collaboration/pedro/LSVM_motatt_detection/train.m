function model = train(name, cachedir, model, pos, neg, warp, randneg, iter, ...
                       negiter, maxnum, keepsv, overlap, cont, phase, C, J)

% model = train(name, model, pos, neg, warp, randneg, iter,
%               negiter, maxsize, keepsv, overlap, cont, C, J)n
% Train LSVM.
%
...
tmpdir_note = cachedir;
if ~exist(tmpdir_note, 'dir')
    mkdir(tmpdir_note);
end;
hdrfile = fullfile(tmpdir_note, [name '.hdr']);
datfile = fullfile(tmpdir_note, [name '.dat']);
modfile = fullfile(tmpdir_note, [name '.mod']);
inffile = fullfile(tmpdir_note, [name '.inf']);
lobfile = fullfile(tmpdir_note, [name '.lob']);
cmpfile = fullfile(tmpdir_note, [name '.cmp']);
objfile = fullfile(tmpdir_note, [name '.obj']);

labelsize = 5;  % [label id level x y]
negpos = 0;     % last position in data mining
maxoverlap = 0.49;  % if using positive images for training, this is the max overlap we allow a 
                    % training example to have with the groundtruth objects
imresizefactor=1.;                    
...
for t = 1:iter
  ...
  for tneg = 1:negiter
    ...
    if datamine
      ...
      fid = fopen(datfile, 'a');
      if randneg > 0
        num = num + negrandom(name, t, model, randneg, neg, maxnum-num, fid, maxoverlap,imresizefactor);
        randneg = randneg - 1;
      else
        [numadded, negpos, fusage, scores, complete] = ...
            neghard(name, tneg, negiter, model, neg, bytelimit, ...
                    fid, negpos, maxnum-num, maxoverlap,imresizefactor);
        num = num + numadded;
        hinge = max(0, 1+scores);
    ...
    if numel(findstr(os, 'windows'))
        cd('C:\science\code\LSVM_modified');
    ...
    % cache model
    save(fullfile(cachedir, [name '_model_' phase '_' num2str(t) '_' num2str(tneg) '.mat']), 'model');
...
%globals;
numpos = length(pos);
[warped, warpinfo] = warppos(model, pos);
...
for i = 1:numpos
  ...
  if isfield(model, 'useextrafeat') & model.useextrafeat == 1
     [impath, imname, ext] = fileparts(pos(i).im);
     feat2 = morefeatures(imreadx(pos(i)), impath, imname, model.sbin, pos(i).flip, [], size(feat, 1), size(feat, 2), warpinfo(i));
     feat(:,:,size(feat,3)+1:size(feat,3)+size(feat2, 3)) = feat2;
  end;
...
for i = 1:batchsize:numpos
  ...
  % data for batch
  data = {};
  parfor k = 1:thisbatchsize   %parfor
    j = i+k-1;
    ...
    imor = im;
    [im, bbox, oldbbox] = croppos(im, bbox);
    if ~isfield(model, 'useextrafeat') | model.useextrafeat == 0
       pyra = featpyramid(im, model);
    else
       [impath, imname, ext] = fileparts(pos(j).im);
       pyra = featpyramid_morefeat(imor, model, impath, imname, pos(j).flip, oldbbox); 
    end;
...
function [num, j, fusage, scores, complete] ...
  = neghard(name, t, negiter, model, neg, maxsize, fid, negpos, maxnum, maxoverlap,imresizefactor)
model.interval = 4;
fusage = zeros(model.numfilters, 1);
numneg = length(neg);
num = 0;
...
for i = 1:batchsize:numneg
  ...
  parfor k = 1:thisbatchsize %parfor
    ...
    res=0;
    if max(size(im,1),size(im,2))<800 & imresizefactor > 1, im = imresize(im,imresizefactor); res=1; end;
    ispos = ispositive(neg(j));
    if ~isfield(model, 'useextrafeat') | model.useextrafeat == 0
       pyra = featpyramid(im, model);
    else
       [impath, imname, ext] = fileparts(neg(j).im);
       pyra = featpyramid_morefeat(im, model, impath, imname, neg(j).flip); 
    end;
    [dets, bs, info] = gdetect(pyra, model, -1.002);
    if numel(dets) & ispos & isfield(neg, 'bboxes')
        % if we're using a positive image, we have to filter out positive
        % detection windows
        posdet = zeros(size(dets, 1), 1);
        bboxes = neg(j).bboxes;
        if res, bboxes = bboxes*imresizefactor; end;
        for j=1:size(bboxes, 1)
            box = bboxes(j, :);
            o = boxoverlap(dets(:, 1:4), box);
            indpos = find(o > maxoverlap);
            if numel(indpos)
                posdet(indpos) = 1;
            end;
        end;
        indneg = find(posdet==0);
        if numel(indneg)
            dets = dets(indneg, :);
            bs = bs(indneg, :);
            info = info(:, :, indneg);
...
function num = negrandom(name, t, model, c, neg, maxnum, fid, maxoverlap, imresizefactor)
numneg = length(neg);
rndneg = floor(maxnum/numneg);
rndnegoverhead = rndneg * 5;
fi = model.symbols(model.rules{model.start}.rhs).filter;
...
for i = 1:numneg
  ...
  if isfield(model, 'useextrafeat') & model.useextrafeat == 1
     [impath, imname, ext] = fileparts(neg(i).im);
     feat2 = morefeatures(im, impath, imname, model.sbin, neg(i).flip, [], size(feat, 1), size(feat, 2));
     feat(:,:,size(feat,3)+1:size(feat,3)+size(feat2, 3)) = feat2;
  end;
  if size(feat,2) > rsize(2) && size(feat,1) > rsize(1)
    negused = 0;
    for j = 1:rndnegoverhead
      if negused < rndneg
          x = random('unid', size(feat,2)-rsize(2)+1);
          y = random('unid', size(feat,1)-rsize(1)+1);
          if ispositive(neg(i))
              [xpixel1, ypixel1] = get_pixel_position(x, y, 1, model, -2, -2);
              [xpixel2, ypixel2] = get_pixel_position(x+rsize(2)-1, y+rsize(1)-1, 1, model, -2, -2);
              box = [xpixel1, ypixel1, xpixel2, ypixel2];
              bboxesgt = neg(i).bboxes;
              if res, bboxesgt = bboxesgt*imresizefactor; end;
              o = boxoverlap(bboxesgt, box);
              if max(o) > maxoverlap
                 continue; 
              end
          end;
          ...
          fwrite(fid, [-1 num+j 0 0 0 2 dim], 'int32');
          fwrite(fid, [obl 1], 'single');
          fwrite(fid, fbl, 'single');
          fwrite(fid, f, 'single');
          negused = negused + 1;
...
function ispos = ispositive(neg)

ispos = 0;
if isfield(neg, 'ispositive') & (neg.ispositive), ispos = 1; end;
