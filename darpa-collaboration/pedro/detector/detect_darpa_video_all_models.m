function detect_darpa_video_all_models(videoname,model_folder,thresh_diff,nms_thresh,model_list)
...
addpath('~/darpa-collaboration/pedro/detector/star-cascade')
    ...
    models = load_models(model_folder,model_list);    % load all models in a folder model_folder
    [padx, pady, sbin, interval] = get_pyrinfo_allmodels(models); % compute image padding, sbin and interval for all models
...
for i = 1 : length(model_files)
	if ~isempty(model_list)
		...
		for j = 1:length(model_list)
			...
			if strncmpi(model_name,model_list{j},length(model_list{j}))
