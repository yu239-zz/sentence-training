function detect_darpa_video_star_cascade_all_models(videoname,model_folder,thresh_diff,nms_thresh,model_list)
...
addpath('~/darpa-collaboration/pedro/detector/star-cascade')
    ...
    for frame=1:length(dir_frames)
        ...
        if (e == 0)
           ...
           for j = 1 : numel(ind)
                i_model = ind(j);
                model_name = csc_models{i_model}.filename;
                fprintf('   model: %s\n', model_name)
                
                if (timeit) th = tic; end;
                [boxes, all] = cascade_detect2(pyra, projpyra, csc_models{i_model}, csc_models{i_model}.thresh + thresh_diff);
                [boxes, all] = get_nms_boxes(im, csc_models{i_model}, boxes, all, nms_thresh);
                if (timeit)
                   t = toc(th);
...
for i = 1 : length(model_files)
	if ~isempty(model_list)
		...
		for j = 1:length(model_list)
			...
			if strcmp(model_name,model_list{j}) || strncmpi([model_name '-'],model_list{j},length(model_list{j}))
