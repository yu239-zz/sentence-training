function [model, pos, neg] = train(name, model, pos, neg, warp, randneg, iter, ...
...
for t = 1:iter
    ...
    if ~cont || t > 1
        ...
        fid = fopen(datfile, 'a');
        if warp > 0
            [numadded,pos] = poswarp(name, t, model, warp, pos, fid);
            fusage = numadded;
        else
            [numadded, fusage, scores, pos] = poslatent(name, t, iter, model, pos, overlap, fid);
        end
        num = num + numadded;
        fclose(fid);
...
%                 keyboard; %pause execution to warn the user
            end
    ...
    for tneg = 1:negiter
        ...
        if datamine
            ...
            fid = fopen(datfile, 'a');
            if randneg > 0
                [ntmp, neg] = negrandom(name, t, model, randneg, neg, maxnum-num, fid);
                num = num + ntmp;
                randneg = randneg - 1;
            else
                [numadded, negpos, fusage, scores, complete, neg] = ...
...
function [num,pos] = poswarp(name, t, model, ind, pos, fid)
% assumption: the model only has a single structure rule
% of the form Q -> F.
globals;
numpos = length(pos);
[warped,pos] = warppos(model, pos);
...
% we create virtual examples by flipping each image left to right
function [num, fusage, scores, pos] ...
...
function [num, j, fusage, scores, complete, neg] ...
...
function [num, neg] = negrandom(name, t, model, c, neg, maxnum, fid)
numneg = length(neg);
