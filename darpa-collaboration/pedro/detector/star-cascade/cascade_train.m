function cascade_train(model, train_folder, modelname, pos, pca)

% cascade_data(model, train_folder, pca, pos)
%
...
% model_cascade_data_pcaK.inf, which is used 
% by cascade_model.m.
%
...
pca_str = num2str(pca);
detfile = fullfile(train_folder, [model.class '_cascade_data_det.inf']);
inffile = fullfile(train_folder, [model.class '_cascade_data_pca' pca_str '.inf']);

%----
if pca == 0
  % file for recording the detection location and component of each
  % detection
...
[num, pos] = process(model, pos, validinfo, pca, detfid, inffid);
...
function [num, pos] = process(model, pos, validinfo, pca, detfid, inffid)
