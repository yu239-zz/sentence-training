function model = cascade_model_2(model, cscdir, pca, thresh)
...
% examples
pca_str = num2str(pca);
inffile = fullfile(cscdir, [model.class '_cascade_data_pca0.inf']);
pcafile = fullfile(cscdir, [model.class '_cascade_data_pca' pca_str '.inf']);
...
pcavals = pcavals(I,:);
pcascores = parse_scores(model, pcavals, pcablocks);
...
for c = 1:model.numcomponents
  ...
  for j = 2:2*n
    X = sum(S(:, j:j+2*n-1), 2);
    m = min(X);
    t{c}(j+2*n-1) = m + model.offsets{o}.w;
  end
