...
(define (show i)
 ...
 (cond ((ppm? i) (write-pnm i "/tmp/show.ppm")
	(system (format #f "feh --force-aliasing /tmp/show.ppm")))
       ((pgm? i) (write-pnm i "/tmp/show.pgm")
	(system (format #f "feh --force-aliasing /tmp/show.pgm")))
	...
	(system (format #f "feh --force-aliasing /tmp/show.png")))
       ;; assume any procedures are from mplotlib
       ((procedure? i) (mplot-run-onscreen i))
...
(define (euclidean-1d-dt-vals v)
 (let* ((n (vector-length v))
	(c-v (malloc (* c-sizeof-double n)))
	(c-v (vector->c-inexact-array c-v v c-sizeof-double #t))
	(c-dt ((c-function pointer ("euclidean_1d_dt_vals" pointer unsigned)) c-v n))
...
(define (euclidean-2d-dt-vals m)
 (let* ((row-dt (map-vector (lambda (row) (euclidean-1d-dt-vals row)) m))
	(column-dt (map-vector (lambda (col) (euclidean-1d-dt-vals col)) (transpose row-dt))))
  (transpose column-dt)))
...
(define (ppm->imlib-image ppm)
 (let ((buf (pnm->pnm-buffer! ppm #t)))
  (let ((i (imlib:create-using-copied-data (pnm-buffer-width buf)
...
(define (image->pnm-buffer! image)
 (cond ((pnm? image) (pnm->pnm-buffer! image #f))
       ((imlib-image? image) (imlib-image->pnm-buffer! image))
       (else (fuck-up))))
...
;; TODO finalizers
(define imlib:load-immediately!
 (c-function imlib-image ("imlib_load_image_immediately" string)))
(define (imlib:save image filename)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_save_image" string)) filename)
 image)
(define (imlib:data-ptr-read-only image)
 (imlib-context-set-image! image)
 ((c-function pointer ("imlib_image_get_data_for_reading_only"))))
(define (imlib:data-ptr image)
 (imlib-context-set-image! image)
 ((c-function pointer ("imlib_image_get_data"))))

;; TODO finalizers
(define (imlib:create-cropped-scaled image sx sy sw sh dw dh)
 (imlib-context-set-image! image)
 ((c-function imlib-image ("imlib_create_cropped_scaled_image" int int int int int int))
  sx sy sw sh dw dh))
(define (imlib:create-scaled image dw dh)
 (imlib-context-set-image! image)
 ((c-function imlib-image ("imlib_create_cropped_scaled_image" int int int int int int))
  0 0 (imlib:width image) (imlib:height image) dw dh))
;; TODO finalizers
(define (imlib:create-rotated image radians)
 (imlib-context-set-image! image)
 ((c-function imlib-image ("imlib_create_rotated_image" double)) radians))
;; TODO finalizers
;; The memory for this image is uninitialized!
(define imlib:create
 (c-function imlib-image ("imlib_create_image" int int)))
;; TODO finalizers
(define imlib:create-using-data
 (c-function imlib-image ("imlib_create_image_using_data" int int pointer)))
;; TODO finalizers
(define imlib:create-using-copied-data
 (c-function imlib-image ("imlib_create_image_using_copied_data" int int pointer)))
;; TODO finalizers
...
(define (imlib:free-and-decache image)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_free_image_and_decache"))))

(define (imlib:draw-pixel image x y color)
 (imlib-context-set-color-rgb/a! color)
 (imlib-context-set-image! image)
 ((c-function pointer ("imlib_image_draw_pixel" int int int)) x y 1)
 image)
(define (imlib:set-has-alpha! image alpha?)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_set_has_alpha" bool)) alpha?)
 image)
(define (imlib:draw-rectangle image x1 y1 w h color)
 (imlib-context-set-color-rgb/a! color)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_draw_rectangle" int int int int)) x1 y1 w h)
 image)
(define (imlib:fill-rectangle image x1 y1 w h color)
 (imlib-context-set-color-rgb/a! color)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_fill_rectangle" int int int int)) x1 y1 w h)
 image)
(define (imlib:draw-ellipse image x y a b color)
 (imlib-context-set-color-rgb/a! color)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_draw_ellipse" int int int int)) x y a b)
 image)
(define (imlib:fill-ellipse image x y a b color)
 (imlib-context-set-color-rgb/a! color)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_fill_ellipse" int int int int)) x y a b)
 image)
(define (imlib:draw-line image color x1 y1 x2 y2)
 (imlib-context-set-image! image)
 (imlib-context-set-color-rgb/a! color)
 ((c-function pointer ("imlib_image_draw_line" int int int int int))
  x1 y1 x2 y2 0)
 image)
(define (imlib:text-draw image x y color string direction radians)
 (imlib-context-set-image! image)
 (imlib-context-set-color-rgb/a! color)
 (imlib-context-set-direction direction)
 (imlib-context-set-angle radians)
 ((c-function void ("imlib_text_draw" int int string)) x y string)
 image)
(define (mlib:text-dimension string radians direction)
 (imlib-context-set-direction direction)
 (imlib-context-set-angle radians)
 ((c-function pointer ("imlib_get_text_dimension" string)) string))

(define (imlib:blend target source merge-alpha? sx sy sw sh dx dy dw dh)
 (imlib-context-set-image! target)
 ((c-function void ("imlib_blend_image_onto_image" imlib-image bool int int int int int int int int))
  source merge-alpha? sx sy sw sh dx dy dw dh)
 target)
...
(define (imlib:dimensions image) (vector (imlib:width image) (imlib:height image)))

;; TODO finalizers
(define imlib:load-font!
 (c-function pointer ("imlib_load_font" string)))
(define (imlib:free-font font)
...
(define (imlib:render-on-drawable image drawable x y)
 (imlib-context-set-image! image)
 (imlib-context-set-drawable! drawable)
 (imlib-context-set-display! (cdr *display*))
 ((c-function void ("imlib_render_image_on_drawable" int int)) x y)
 image)

(define imlib:fill-polygon
...
(define (imlib:flip-vertical image)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_flip_vertical"))))
(define (imlib:flip-horizontal image)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_flip_horizontal"))))

(define ffmpeg-video-frame-data-as-imlib
 (c-function imlib-image ("ffmpeg_get_frame_as_imlib" pointer)))

(define (imlib:create-rotated-with-background image radians background-color)
 (let*
   ((rotated (imlib:create-rotated image radians))
    (background (imlib:create (imlib:width rotated) (imlib:height rotated))))
  (imlib:fill-rectangle
   background 0 0 (imlib:width background) (imlib:height background)
   (vector (r background-color) (g background-color) (b background-color) 255))
  (imlib:blend background rotated #t
               0 0 (imlib:width rotated) (imlib:height rotated)
               0 0 (imlib:width rotated) (imlib:height rotated))
  (imlib:set-has-alpha! background #f)
  (imlib:free rotated)
  background))

(define (imlib:create-cropped image sx sy sw sh)
 (imlib:create-cropped-scaled image sx sy sw sh sw sh))

(define (imlib:create-with-cropped-background image background-color)
 (define (for-each-m-n* f m n)
  (if (< m n)
      (for-each-m-n f m n)
      (for-each-m-n-dec f m n)))
 (let ((x1 (call-with-current-continuation
            (lambda (k)
             (for-each-m-n*
              (lambda (x)
               (for-each-m-n*
                (lambda (y)
                 (unless (equal? (imlib-color-at x y) background-color) (k x)))
                0 (- (imlib:height image) 1)))
              0 (- (imlib:width image) 1)))))
       (y1 (call-with-current-continuation
            (lambda (k)
             (for-each-m-n*
              (lambda (y)
               (for-each-m-n*
                (lambda (x)
                 (unless (equal? (imlib-color-at x y) background-color) (k y)))
                0 (- (imlib:width image) 1)))
              0 (- (imlib:height image) 1)))))
       (x2 (call-with-current-continuation
            (lambda (k)
             (for-each-m-n*
              (lambda (x)
               (for-each-m-n*
                (lambda (y)
                 (unless (equal? (imlib-color-at x y) background-color) (k x)))
                0 (- (imlib:height image) 1)))
              (- (imlib:width image) 1) 0))))
       (y2 (call-with-current-continuation
            (lambda (k)
             (for-each-m-n*
              (lambda (y)
               (for-each-m-n*
                (lambda (x)
                 (unless (equal? (imlib-color-at x y) background-color) (k y)))
                0 (- (imlib:width image) 1)))
              (- (imlib:height image) 1) 0)))))
  (imlib:create-cropped image x1 y1 (- x2 x1) (- y2 y1))))

;; Low-level parts of the old API still in internal use
(define imlib-context-set-display!
 (c-function void ("imlib_context_set_display" pointer)))
(define imlib-context-set-blend!
 (c-function void ("imlib_context_set_blend" int)))
...
(define imlib-context-set-anti-alias!
 (c-function void ("imlib_context_set_anti_alias" bool)))
...
(define imlib-context-set-operation (c-function void ("imlib_context_set_operation" int)))
...
(define imlib-op-copy (c-value int "IMLIB_OP_COPY"))
(define imlib-op-add (c-value int "IMLIB_OP_ADD"))
(define imlib-op-subtract (c-value int "IMLIB_OP_SUBTRACT"))
(define imlib-op-reshade (c-value int "IMLIB_OP_RESHADE"))

;;; This API is DEPRECATED
;;; See the very similar imlib:* API
...
(define imlib-create-cropped-scaled-image
 (c-function imlib-image ("imlib_create_cropped_scaled_image" int int int int int int)))
(define imlib-create-rotated-image
 (c-function imlib-image ("imlib_create_rotated_image" double)))
;; The memory for this image is uninitialized!
(define imlib-create-image
 (c-function imlib-image ("imlib_create_image" int int)))
(define imlib-create-image-using-data
 (c-function imlib-image ("imlib_create_image_using_data" int int pointer)))
(define imlib-create-image-using-copied-data
 (c-function imlib-image ("imlib_create_image_using_copied_data" int int pointer)))
(define imlib-clone-image
 (c-function imlib-image ("imlib_clone_image")))
(define imlib-free-image
...
(define imlib-image-flip-vertical
 (c-function void ("imlib_image_flip_vertical")))
(define imlib-image-flip-horizontal
 (c-function void ("imlib_image_flip_horizontal")))
(define imlib-image-blur (c-function void ("imlib_image_blur" int)))
(define (imlib:blur image radius)
 (imlib-context-set-image! image)
 ((c-function void ("imlib_image_blur" int)) radius))
