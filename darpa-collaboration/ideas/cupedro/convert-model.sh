...
while (( $# > 0 )) ; do
   ...
   if [ "$FILE" = "--pca" ] ; then
	 (( $# == 0 )) && echo "Expected a filename after --pca." 1>&2 && exit1
	 FILE="$1"
	 PCA=1
	 shift
   fi

   ! [ -e "$FILE" ] && echo "File not found: $FILE" 1>&2 && exit 1
...
cd $TMPD
matlab -nodesktop -nosplash -nojvm -r "convert"
