...
(define c-int-vector-ref
 (c-function int ("c_int_vector_ref" pointer unsigned)))

(define free-c-vector!
 (c-function void ("free_c_vector" pointer)))
...
(c-define-struct-field "IVecStruct" "x" unsigned)

(c-define-struct-field "IVecStruct" "v" pointer)
...
(define (psi-uu psi) (vector-length (psi-b psi)))
...
(define (model->psi features model)
 (make-psi (c-string->symbol (model-name model))
 	   (model-pos model)
 	   features
 	   (model-n-roles model)	  
