...
(define *sentence-tracker?* #f)
(define *sentence-roles* #f)
(define *machines-roles* #f)
(define *sentence-state-machines* '())
(define *sentence* #f)
(define *sentence-videos* #f)
(define *sentence-models* #f)
(define *machine-type* #f)
(define *colour-boxes-on-the-fly?* #f)
(define *viterbi-sentence-track-in-c?* #f)
...
(define *sentence:cfg* #f)
(define *sentence:semantics* #f)
(define *sentence:state-machines* #f)
(define *sentence:feature-medoids* #f)
(define *sentence:objects* #f)
...
(define-structure global-option
 gui?
 interval
 alpha beta kappa lookahead  ;; THIS IS FOR BOX PREDICTION, NOT FOR FEATURE COMPUTATION !
...
(define (create-default-global-options)
 (make-global-option #t ; gui?
		     #f ; interval
                     10 15 1 2 ; Alpha beta kappa lookahead
...
(define (global-options->pretty-string options)
 (apply
  ...
  (list
   ...
   (format #f "Interval:                    ~a~%" (global-option-interval options))
   (format #f "Alpha:                        ~a~%" (global-option-alpha options))
   (format #f "Beta:                         ~a~%" (global-option-beta options))
   (format #f "Kappa:                        ~a~%" (global-option-kappa options))
   ...
   (format #f "normalized-box-strength:      ~a~%" (global-option-normalized-box-strength? options))
...
(define (interval) (global-option-interval *parameters*))
(define (alpha) (global-option-alpha *parameters*))
(define (beta) (global-option-beta *parameters*))
(define (kappa) (global-option-kappa *parameters*))
...
(define (normalized-box-strength?) (global-option-normalized-box-strength? *parameters*))
...
(define (write-captioned-and-tracked-videos video)
 ...
 (let* ((named-track-boxes
   ...
   *caption-height*
   ...
   (rank-box-colors?)))
 (let ((destination-directory (if (video-destination-directory) (video-destination-directory) ".")))
  (when (not (stop-before-tracker?))
   (ffmpeg
    (if (and (downsample-fps) (not (= (downsample-fps) 0))) (downsample-fps) *video-fps*)
    (generic-root-pathname video (format #f "/%06d/~a.png" "tracked"))
    (format #f "~a/~a~a-tracked.avi" destination-directory
	    (any-video->string video)
	    (if *sentence*
		(string-append "[" (lexicalize-phrase *sentence* *sentence:cfg*) "]")
		""))))
  (if (null? *sentences*)
      ...
      (begin
       ...
       (when (verbose?)
	(for-each (lambda (sentence) (format #t "~a ~a~%" (first sentence) (second sentence)))
		  *sentences*))
       (render-sentence-frames video (first *sentences*) "captioned" (rank-box-colors?))
       (ffmpeg (if (and (downsample-fps) (not (= (downsample-fps) 0))) (downsample-fps) *video-fps*)
	       (generic-root-pathname video (format #f "/%06d/~a.png" "captioned"))
	       (format #f "~a/~a-captioned.avi" destination-directory (any-video->string video))))))
 (add-on-exit-thunk!
...
(define (box-projection-transformation)
...
(define (box-coherence-transformation)
 ...
 (let loop ()
  ...
  (when (or (not *display*) (not (xpending-events?)))
   (when (and *processing-state* *video*)
    ;; A FSM
    ;;
    ;; start -#t> next-frame -(= *frame-number* *video-length*)>
    ;;  (|
    ;;   -viterbi-event-tracker?> #(event-track model-groups hmms)
    ;;   -viterbi-tracker?> #(track model-groups) -(null? model-groups)> likelihoods)
    ;;   -#t> sentences -#t> done
    ;;
    ;; play -(= *frame-number* *video-length*)> done
    (cond
     ((equal? *processing-state* 'done)
      (stop-callbacks!)
      (when *fsm-start-time*
     ...
     ((equal? *processing-state* 'next-frame)
      ...
      (when (>= *frame-number* *video-length*)
       ...
       (set! *processing-state*
             ...
             ;; order
             (vector (if *sentence-tracker?* 'sentence-track 'track)
                     (reverse (model-groups)))))
      (unless (ffmpeg-video-finished? *ffmpeg-video*)
       (ffmpeg-next-frame! *ffmpeg-video*))
      (next))
     ;; STATE = sentence-track, but we want to stop
     ((and (vector? *processing-state*)
	 (equal? (x *processing-state*) 'sentence-track)
	 (stop-before-tracker?))
      (user-feedback! "Not tracking")
      (set! *processing-state* 'done)
      (process-write-commands)
      (next))
     ;; STATE = sentence-track
     ((and (vector? *processing-state*)
	 (equal? (x *processing-state*) 'sentence-track))
      (when *colour-boxes-on-the-fly?*
       (format #t "Colouring boxes~%")
       (set!
	*detector-boxes-movies*
	(map
	 (lambda (boxes-movie)
	  (colour-boxes-movie *video* boxes-movie (scale)))
	 *detector-boxes-movies*)))
      ;; TODO Filter by model name
      (let
        ((result
                  ...
                  (lambda (model-group)
                   (first
                    (viterbi-prepare-tracks
                     (scale) (video-dimensions *video*) (video-length *video*)
                     ...
                     *profile-best-boxes?*
                     (lookahead))))
                  (y *processing-state*))))
	   ;; needed because a predicted-boxes-movie represents a model group
	   ;; not an individual model
	   (set! *predicted-boxes-movies* processed-tracks)
           ;; TODO This is bad
           (display
	    (list (* (if (downsample?) (/ (downsample-fps) 30) (/ *video-fps* 30)) (alpha))
		  (* (if (downsample?) (/ (downsample-fps) 30) (/ *video-fps* 30)) (beta))))
           (newline)
           (timed-f
            increment-tracker-timing
            (lambda ()
	     (if *viterbi-sentence-track-in-c?*
		 (viterbi-sentence-track-in-c
		  *sentence-state-machines*
		  (+ (maximum (join *machines-roles*)) 1)
		  *machines-roles*
		  (map join
		       (transpose-list-of-lists
			(map (lambda (movie model-group)
			      (map (lambda (boxes)
				    (if (null? boxes)
					;; when there is no box at some frame
					(pad-with-last-if-necessary
					 (list (make-garbage-box (first model-group))) (top-n))
					(pad-with-last-if-necessary boxes (top-n))))
				   movie))
			     (reselect-topn-boxes processed-tracks (top-n))
			     (y *processing-state*))))
		  (box-coherence-transformation)
		  (scale)
		  *sentence:objects*
		  *sentence:feature-medoids*
		  #t)
		 (viterbi-fast-sentence-track-one
		  (transpose-list-of-lists
		   ;;list-of-boxes-movies -> objects-movie
		   (fill-sentence-tracker-roles
		    (reselect-topn-boxes processed-tracks (top-n))
		    (y *processing-state*)
		    *sentence-models*))
		  (box-coherence-transformation)
		  ;; TODO This is bad
		  (* (if (downsample?) (/ (downsample-fps) 30) (/ *video-fps* 30)) (alpha))
		  ;; TODO This is bad
		  (* (if (downsample?) (/ (downsample-fps) 30) (/ *video-fps* 30)) (beta))
		  (kappa)
		  (scale)
	 ...
	 (list *sentence* #f #f #f)
	 ;; (sentence-specific-data-filename *video* *sentence*)
	 (sentence-data-filename *video*)
	 ))
       (when result
	(when *viterbi-sentence-track-in-c?*
	 (format #t "Sentence tracker cost ~a~%" (second result))
	 (format #t "movie-length ~a~%" (length (first result)))
	 (write-object-to-file
	  (list *sentence* (second result) *sentence-roles*)
	  ;; (sentence-specific-data-filename *video* *sentence*)
	  (sentence-data-filename *video*)))
	(when (not *viterbi-sentence-track-in-c?*)
	 (format #t "Sentence tracker cost ~a~%" (first (third result)))
	 (format #t "Single box cost ~a~%" (second (third result)))
	 (format #t "Pair box cost ~a~%" (third (third result)))
	 (format #t "movie-length ~a~%" (length (first result)))
	 (format #t "Features~%")
	 (pp (fourth result)) (newline)
	 (format #t "States~%")
	 (pp (map-indexed (lambda (r i) (list i r)) (second result))) (newline)
	 (write-object-to-file
	  (list *sentence* (third result) (second result) *sentence-roles* (fourth result))
	  ;; (sentence-specific-data-filename *video* *sentence*)
	  (sentence-data-filename *video*)))
	(let ((a (transitive-equivalence-classesp
		  (lambda (a b) (equal? (second a) (second b)))
		  (zip (transpose-list-of-lists (first result))
		       *sentence-models*))))
	 (set! *tracked-movies-names* (map (o second first) a))
	 (set! *tracked-boxes-movies-groups* (map (lambda (g) (map first g)) a)))
	(set! *smooth-tracked-boxes-movies-groups* *tracked-boxes-movies-groups*)
	(set! *processing-state* 'done)
	(process-write-commands)
	(when (write-tracker-to-disk?)
	 (when (verbose?) (format #t "Writing tracker results to disk~%"))
	 (timed-f
	  increment-saving-timing
	  (lambda ()
	   (for-each-indexed
	    (lambda (box-movies i)
	     (for-each-indexed
	      (lambda (box-movie j)
	       (write-voc4-sentence-tracked-box-movie
		box-movie *video* (list-ref *tracked-movies-names* i)
		(number->string (+ j 1))))
	      box-movies))
	    *tracked-boxes-movies-groups*))))
...
(define (set-video-and-parameters! parameters)
 ...
 (when (global-option-interval parameters)
  (let ((start (first (global-option-interval parameters)))
	(end (second (global-option-interval parameters))))
   (unless (< start end)
    (panic "Invalid temporal interval start:~a end:~a~%" start end))
   (set-video-start! *video* start)
   (set-video-end! *video* end)))
 (debug-message! (format #f "setting video and parameters: ~a~%" *video*))
 ...
 (if (equal? #f *video*)
     ...
     (let ((video-info-list (video-info *video*)))
      ...
      (unless (or *sentence-tracker?* (equal? "" (event-models-file)))
	...
	(read-object-from-file
	 (string-append (if (has-directory? (event-models-file))
      ...
      (when (verbose?)
       ...
       (format #t "~%"))
      (when (> *video-fps* 30)
       (panic "video-fps is higher than 30, please downsample the video first"))
      (reset-process!)
      #t)))
...
(define (create-demo-parameters video-path model-groups)
 (let ((options (create-default-global-options)))
  ...
  (set-global-option-kappa! options (kappa))
  ...
  (set-global-option-normalized-box-strength?! options (normalized-box-strength?))
...
(define-command
 (main
  ...
  (at-most-one ("interval" interval? (interval "start,end" string-argument "")))
  (at-most-one ("machine-path" machine-path? (machine-path "path" string-argument "")))
  ...
  (at-most-one ("sentence-tracker-inference" sentence-tracker-inference?
		(inference-sentence "sentence" string-argument ""))
	       ("sentence-tracker-retrieval" sentence-tracker-retrieval?
		(retrieval-sentence "sentence" string-argument ""))
	       ("sentence-tracker-generation" sentence-tracker-generation?
		(generation-sentences "filename" string-argument ""))
               ("verb" verb?
                (verb "filename" string-argument "")
                (participants "participants" integer-argument 1))
	       ("event-models-file"
		event-models-file?
		(event-models-file "pathname" string-argument "")))
  (at-most-one ("sentence-tracker-pos" sentence-tracker-pos?
		(sentence-pos "pos" string-argument "S")))
  (at-most-one ("toy-corpus-cfg" toy-corpus-cfg?)
	       ("the-306-corpus-cfg" the-306-corpus-cfg?)
               ("needle-in-a-haystack-cfg" needle-in-a-haystack-cfg?)
	       ("new3-corpus-cfg" new3-corpus-cfg?)
	       ("new4-corpus-cfg" new4-corpus-cfg?))
  (at-most-one ("viterbi-sentence-track-in-c" viterbi-sentence-track-in-c?))
  (at-most-one ("fsms" state-machines-fsms?)
	       ("hmms" state-machines-hmms?))
  (at-most-one ("colour-boxes-on-the-fly" colour-boxes-on-the-fly?))
  (at-most-one ("normalized-box-strength" normalized-box-strength?))
  (at-most-one
  ...
  (at-most-one ("kappa" kappa? (kappa "kappa" real-argument 1)))
 ...
 (when (or sentence-tracker-retrieval? sentence-tracker-generation?)
  (panic "These are currently run as inference tasks, but parallelly in batch mode"))
 (when sentence-tracker-inference?
  (set! *sentence-tracker?* #t)
  (set! *sentence:cfg*
        (cond (toy-corpus-cfg? *toy-corpus:cfg*)
	      (the-306-corpus-cfg? *toy-corpus:cfg*)
	      (needle-in-a-haystack-cfg? *needle-in-a-haystack:cfg*)
	      (new3-corpus-cfg? *new3-corpus:cfg*)
	      (new4-corpus-cfg? *new4-corpus:cfg*)
              (else (panic "Must specify a language"))))
  (set! *sentence:semantics*
        (cond (toy-corpus-cfg? *toy-corpus:semantics*)
	      (the-306-corpus-cfg? *toy-corpus:semantics*)
              (needle-in-a-haystack-cfg? *needle-in-a-haystack:semantics*)
	      (new3-corpus-cfg? *new3-corpus:semantics*)
	      (new4-corpus-cfg? *new4-corpus:semantics*)
              (else (panic "Must specify a language"))))
  (set! *sentence:feature-medoids*
	(cond (toy-corpus-cfg? (fuck-up))
	      (the-306-corpus-cfg? *306-corpus:feature-medoids*)
	      (needle-in-a-haystack-cfg? (fuck-up))
	      (new3-corpus-cfg? *new3-corpus:feature-medoids*)
	      (new4-corpus-cfg? *new4-corpus:feature-medoids*)
	      (else (panic "Mush specify a language"))))
  (set! *sentence:state-machines*
	(if machine-path?
	    (map (lambda (psi-model)
		  (make-trained-hmm (psi-name psi-model) '()
				    (vector-length (psi-parameters psi-model))
				    0 (psi->model psi-model) 2 #f 'trained))
		 (read-object-from-file machine-path))
	    (cond (toy-corpus-cfg?
		   (if state-machines-hmms?
		       *toy-corpus:hmms*
		       (fuck-up)))
		  (the-306-corpus-cfg?
		   (if state-machines-hmms?
		       (*306-corpus:hmms*)
		       (fuck-up)))
		  (needle-in-a-haystack-cfg?
		   (if state-machines-hmms? (fuck-up) *needle-in-a-haystack:fsms*))
		  (new3-corpus-cfg?
		   (if state-machines-hmms?
		       (*new3-corpus:hmms*)
		       *new3-corpus:fsm*))
		  (new4-corpus-cfg?
		   (if state-machines-hmms?
		       ...
		       (fuck-up)))
		  (else (panic "Must specify a language")))))
  (set! *machine-type* (if state-machines-hmms? 'hmm 'fsm))
  (set! *sentence:objects*
	(cond (toy-corpus-cfg? *toy-corpus:objects*)
	      (the-306-corpus-cfg? *306-corpus:objects*)
	      (needle-in-a-haystack-cfg? #f) ;; FIXME , to specify a list
	      (new3-corpus-cfg? *new3-corpus:objects*)
	      (new4-corpus-cfg? *new4-corpus:objects*)
	      (else (panic "Must specify a language"))))
  (set! *viterbi-sentence-track-in-c?* viterbi-sentence-track-in-c?)
  (when colour-boxes-on-the-fly? (set! *colour-boxes-on-the-fly?* #t))
  (format #t "Color boxes on the fly? ~a~%" *colour-boxes-on-the-fly?*)
  (set! *sentence* inference-sentence)
  (format #t "Sentence: ~a~%" *sentence*)
  (when (null? model-names-groups) (panic "-m is required"))

  (let ((participants-roles-and-state-machines
  	 (sentence:sentence->participants-roles-and-state-machines
  	  *sentence* *sentence:cfg* *sentence:semantics* *sentence:state-machines*)))
   (set! *sentence-state-machines* (first participants-roles-and-state-machines))
   (set! *sentence-models* (map second (second participants-roles-and-state-machines)))
   (set! *sentence-roles* (second participants-roles-and-state-machines))
   (set! *machines-roles*  (third participants-roles-and-state-machines)))
  
  (pp *sentence-models*) (newline)
  (pp *sentence-roles*) (newline)
  (pp *sentence-state-machines*) (newline))
 ...
 (let* ((final-cascade-adjustment (if matlab-object-detector?
	...
	(model-groups
	 (cond
	  ((or model-names-groups? *sentence-tracker?*)
  ...
  (apply (if gui? run-from-gui! run-from-command-line)
         (list cameras verbose?
               (make-global-option gui?
				   (and interval?
				      (map string->number (pregexp-split "," interval)))
				   alpha beta kappa look-ahead
