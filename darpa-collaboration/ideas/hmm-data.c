...
int normalizeFFM(Ffm *m, enum training_mode train_mode){
  assert(m);
  int i;
  for (i = 0; i < m->ii; i ++)
    if (m->ft[i] == FT_DISCRETE)
      if (normalizeDiscrete((DParam *)m->p[i], train_mode) == FALSE)
	return FALSE;
  return TRUE;
}
...
{ DParam *p = (DParam *)param;
  ...
  Real scale = 0.0, corrected_scale, *w, *dtf = safe_malloc(p->kk * sizeof *dtf);
  ...
  if (fabs(scale)>1e-50) {
	...
	return FALSE;
      p->p[k] /= scale;
      if(train_mode == HMM_DT && p->p[k] != 0.0 && p->p[k] < CORRECTION_EPS){
      	fprintf(stderr, "p->p[%d]=%e is replaced by 0.0!\n", k, p->p[k]);
      	p->p[k] = 0.0;
      	flag = 1;
      }
...
int normalizeDiscrete(DParam *p, enum training_mode train_mode){
  int k, flag;
  Real scale = 0, corrected_scale;
  for (k = 0; k < p->kk; k ++)
    scale += p->p[k];
  if (scale != 0.0) {
    flag = 0;
	...
	return FALSE;
	 ...
	 && p->p[k] != 0.0 && p->p[k] < CORRECTION_EPS){
	printf("normaliseDiscrete: clip zero\n");
      	p->p[k] = 0.0;
      	flag = 1;
      }
      corrected_scale += p->p[k];
    }
    assert(corrected_scale != 0);
    if(flag){
      for(k= 0; k < p->kk; k ++)
    	p->p[k] /= corrected_scale;
    }
    ...
    fprintf(stderr, "normalizeDiscrete(): Sum of weights == 0\n");
    return FALSE;
  }
  return TRUE;
}
...
Real FFM_logL_one(Ffm *m, RVec data){
  int i, ii = m->ii;
  void **p = m->p;
  Real l = 0.0;
  Real *x = VECTOR(data);
