...
(define (last-vector v) (vector-ref v (- (vector-length v) 1)))
...
(define (smooth-max-chks-2 a b t)
 (cond ((or (and (= infinity a) (= infinity b))
	    (and (= minus-infinity a)  (= minus-infinity b)))
	(/ (+ a b t) 2))
       ((or (= minus-infinity a) (= infinity b))
	(lift-real*real->real
	 (lambda (a b) b)
	 (lambda (a b) 0) ;; df/da
	 (lambda (a b) 1) ;; df/db
	 a b))
       ((or (= infinity a) (= minus-infinity b))
	(lift-real*real->real
	 (lambda (a b) a)
	 (lambda (a b) 1) ;; df/da
	 (lambda (a b) 0) ;; df/db
	 a b))
       (else (/ (+ (sqrt (+ (sqr (- a b)) (sqr t))) a b) 2))))
...
;; This produces a continous function for AD
(define (my-add-exp1 e1 e2)
 (let* ((e-max (smooth-max-chks-2 e1 e2 1e-30))
	(e-min (smooth-min-chks-2 e1 e2 1e-30)))
  (if (= e-max minus-infinity)
      (lift-real*real->real
       (lambda (a b) minus-infinity)
       (lambda (a b) 1)
       (lambda (a b) 1)
       e-min e-max)
      (+ (log (+ (exp (- e-min e-max)) 1.0)) e-max))))
...
(define (safe-read-file filename . default)
 (if (file-exists? filename)
     (read-file filename)
     (if (null? default)
	 (panic "~a does not exist!" filename)
	 (car default))))

;;; Lists & Vectors

(define (lookup i g) (assoc i g))
(define (lookup-value i g)
 (let ((a (assoc i g))) (and a (second a))))
(define (lookup-values i g)
 (let ((a (assoc i g))) (and a (rest a))))

(define (split-n n l)
 (if (null? l) l (cons (take n l) (split-n n (drop n l)))))

(define (split-comma s) (pregexp-split "," s))
...
(define (sum-f f l) (map-reduce + 0 f l))
(define (sum-vector v) (reduce-vector + v 0))
(define (sum-vector-f f v) (map-reduce-vector + 0 f v))
